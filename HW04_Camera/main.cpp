...
// imGuIZMO가 내부 수학 타입으로 glm을 쓰도록 설정
// 이 줄이 없으면 imGuIZMO의 quat(vgm::Quat)와 glm::quat이 섞여 컴파일 에러가 남.
...
////////////////////////////////////////////////////////////////////////////////
ImFont* font = NULL;
std::string font_path = "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc";
...
void compose_imgui_frame()
{
    ...
    // control window
    {
        ...
        ImGui::SliderFloat3("Translate", glm::value_ptr(g_vec_model_translate), -10.0f, 10.0f);
    ...
    // control window
    {
        ...
        ImGui::SliderFloat3("Translate", glm::value_ptr(vec_cam_pos), -10.0f, 10.0f);
...
void scroll_callback(GLFWwindow* window, double x, double y)
{
    ...
    {
        float ortho_scale = g_camera.ortho_scale();
        ortho_scale -= static_cast<float>(y) * 0.1f;

        if (ortho_scale < 0.1f)  ortho_scale = 0.1f;
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    // update camera extrinsic parameter with key-inputs
    const float camera_speed = 0.1f;

    if (action == GLFW_PRESS || action == GLFW_REPEAT)
    {
        if (key == GLFW_KEY_W)
            g_camera.move_forward(camera_speed);
        if (key == GLFW_KEY_S)
            g_camera.move_backward(camera_speed);
        if (key == GLFW_KEY_A)
            g_camera.move_left(camera_speed);
        if (key == GLFW_KEY_D)
            g_camera.move_right(camera_speed);
        if (key == GLFW_KEY_Q)
            g_camera.move_down(camera_speed);
        if (key == GLFW_KEY_E)
            g_camera.move_up(camera_speed);
    }
}
...
void framebuffer_size_callback(GLFWwindow* window, int width, int height)
{
    ...
    g_camera.set_aspect(static_cast<float>(width) / static_cast<float>(height));
...
void set_transform()
{
    ...
    g_mat_model = g_mat_model * glm::translate(glm::mat4(1.0f), g_vec_model_translate);
...
void init_scene()
{
    ...
    g_camera.set_ortho_scale(2.0f);
    g_camera.set_aspect(1.0f);
    g_camera.set_pose(glm::vec3(0.f, 0.f, 5.f),
