...
glm::quat   g_quat_model_rotation;  // R, ImGuIZMO.quat 관련 변수, 쿼터니언 행렬
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
  ...
  if (key == GLFW_KEY_MINUS && action == GLFW_PRESS)
    {
      g_vec_model_scale -= 0.1f;
      g_vec_model_scale = glm::max(g_vec_model_scale, glm::vec3(0.1f));
    }
}
...
void set_transform() 
{
  ...
  glm::mat4 T = glm::translate(glm::mat4(1.0f), g_vec_model_translate);
  glm::mat4 R = glm::mat4_cast(g_quat_model_rotation);
  glm::mat4 S = glm::scale(glm::mat4(1.0f), g_vec_model_scale);
