...
#include "Camera.h"
#include "Object.h"
static quat glm_to_imguizmo_quat(const glm::quat& q)
{
    return quat(q.w, q.x, q.y, q.z);
}

static glm::quat imguizmo_to_glm_quat(const quat& q)
{
...
void compose_imgui_frame()
{
  ...
  // control window
  {
      ...
      glm::quat glm_quat;
      object.get_rotate(glm_quat);
      quat gizmo_quat = glm_to_imguizmo_quat(glm_quat);

      glm::vec3 scale = object.get_scale();

      ImGui::SliderFloat3("Tranlsate", glm::value_ptr(translate), -10.0f, 10.0f);
      ImGui::gizmo3D("Rotation", gizmo_quat);
      ImGui::SliderFloat3("Scale", glm::value_ptr(scale), 0.1f, 100.0f);

      object.set_translate(translate);
      object.set_rotate(imguizmo_to_glm_quat(gizmo_quat));
      object.set_scale(scale);
  ...
  // control window
  {
      ...
      glm::quat glm_quat_cam;
      glm::vec3 vec_cam_pos;

      camera.get_pose(glm_quat_cam, vec_cam_pos);

      quat gizmo_quat_cam = glm_to_imguizmo_quat(glm_quat_cam);

      ImGui::SliderFloat3("Tranlsate", glm::value_ptr(vec_cam_pos), -10.0f, 10.0f);
      ImGui::gizmo3D("Rotation", gizmo_quat_cam);

      camera.set_pose(imguizmo_to_glm_quat(gizmo_quat_cam), vec_cam_pos);
...
void render_scene()
{
    ...
    for (std::size_t i = 0; i < g_objects.size(); ++i)
    {
        ...
        // shader uniform으로 PVM 전달
        glUniformMatrix4fv(
            loc_u_PVM,
            1,
            GL_FALSE,
            glm::value_ptr(mat_PVM)
        );
