...
void render_object()
{
    ...
    glm::quat quat_cam;
    glm::vec3 vec_camera_position;
    camera.get_pose(quat_cam, vec_camera_position);

    glUniformMatrix4fv(loc_u_view_matrix, 1, GL_FALSE, glm::value_ptr(mat_view));

    glUniform3fv(loc_u_camera_position, 1, glm::value_ptr(vec_camera_position));
    ...
    for (std::size_t i = 0; i < g_models.size(); ++i)
    {
        ...
        glm::mat3 mat_normal = glm::mat3(glm::transpose(glm::inverse(mat_model)));
...
{
    GLFWwindow* window = createWindow(1000, 1000, "Phong Reflection");
