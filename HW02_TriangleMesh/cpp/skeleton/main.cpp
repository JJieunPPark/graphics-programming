...
void update_buffer_objects()
{
  ...
  if(g_mesh_model == kCube){
    ...
    g_position_size = sizeof(cube::triangle_soup::position); //::은 std::같이 경로 표시. cube데이터안에 있는 데이터 가져오는거임
  ...
  assert(g_position_size == g_color_size); //position이랑 color 데이터크기가 같아야한다고 선언
  ...
  //GL_STATIC_DRAW : 이 데이터 거의 안 바꿀거고, GPU가 읽기만 할거야
...
void init_buffer_objects()
{
  ...
  // generate VBO & IBO
  glGenBuffers(1, &position_buffer); //gpu에서 1개 만큼의 포지션버퍼 생성
...
void render_object()
{
  ...
  glEnableVertexAttribArray(loc_a_color);//엥 왜 bind먼저하지 > 상관없대 근데 bind 보통 먼저한다함
